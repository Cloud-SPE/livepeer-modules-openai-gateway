 ...
 *
 * This gateway-owned implementation follows the advertised estimator
 * contract independently of the broker's implementation. The point of two
 * implementations is that a client can compute a funding ceiling BEFORE
 * the work runs and the broker can bill from the same media facts
 * afterwards. If they disagree, settlement exceeds the ceiling and a
 * correct exchange is refused.
 */
