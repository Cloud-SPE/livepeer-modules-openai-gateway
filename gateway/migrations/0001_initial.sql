...
-- One unified schema, no incremental tables added by later migrations.
--
-- Mirrors the repo's SaaS-shell concepts (waitlist → api_keys → user_sessions →
-- usage_reservations + models registry cache) but ships as a single
-- migration since this is a greenfield deploy.
...
CREATE TABLE waitlist (
    ...
    -- Verification: tokens stored as SHA-256(token + IP_HASH_PEPPER).
...
-- ── models (pure cache of service-registry snapshot) ──────────────────
CREATE TABLE models (
...
CREATE INDEX idx_models_capability
    ON models (capability, model_id)
    WHERE active = true;
CREATE INDEX idx_models_active
    ON models (active, snapshot_at);
